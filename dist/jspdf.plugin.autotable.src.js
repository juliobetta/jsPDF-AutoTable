/** 
 * jsPDF AutoTable plugin v2.0.38
...
jsPDF.API.autoTableHtmlToJson = function (tableElem, includeHiddenElements) {
    ...
    for (var i = 1; i < tableElem.rows.length; i++) {
        ...
        var _style = window.getComputedStyle(tableRow);
        if (includeHiddenElements || _style.display !== 'none') {
            var rowData = [];
            ...
            try {
                for (var _iterator = Object.keys(columns)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
                    ...
                    var _cell = tableRow.cells[j];
                    var val = _cell ? _cell.textContent.trim() : '';
                    rowData.push(val);
                }
            } catch (err) {
